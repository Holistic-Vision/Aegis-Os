...
// Global consent helper (avoids stuck UI if listener binding fails)
try{
  window.__aegis_acceptConsent = function(){
    try{
      const db2 = loadDB();
      ...
      console.error(e);
      alert("Erreur: impossible d'enregistrer le consentement.");
    }
  };
}catch(e){}
async function ensureConsent(){
  const db = loadDB();
  // Defensive: first run may not have profile object yet
  if(!db.profile){
    db.profile = {};
  ...
  const html = layout(`
    <div class="grid">
      <div class="card">
        ...
        <hr />
        <button class="btn primary" id="acceptBtn" onclick="window.__aegis_acceptConsent && window.__aegis_acceptConsent()">${escapeHtml(get("consent"))}</button>
